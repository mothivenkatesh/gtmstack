...
### The most verifiable GTM agents. Built in public, on the richest GTM context graph.

We cannot win on data volume. Clay, ZoomInfo, and Reo already bought that race, and buying the same data everyone else buys is not a moat. We are not claiming a better harness either: Nevara, Warmly, and Nex all have one, and a context graph plus a learning loop is table stakes in this category now.

**We win by being the ones who can prove it.**

Every GTM agent company publishes outcome claims. Nobody publishes reliability with a methodology. So here is ours, regenerated on every change, and the release gate fails if it regresses:

| | | Target | |
|---|---|---|---|
| Buying-intent precision | **0.952** | 0.85 | pass |
| Buying-intent recall | **1.0** | 0.70 | pass |
| Sentiment accuracy | **0.825** | 0.80 | pass |
| Hard cases (sarcasm, implication) | **0.571** | | **below par, and stated** |

Run it yourself: `python evals/run_evals.py`. The golden set is in the repo. So is the case that still fails.

**Richest means deepest, not biggest.** Every node carries who wrote it, which run produced it, its source, the rule that permitted the action, and what it eventually produced. That graph cannot be bought, because it is generated by running. It is small today, the number below is live, and that is the point of building in public.
...
**73 nodes, 20 edges.** Generated by `scripts/graph_stats.py`, not typed by hand.

| | Count | What it is |
|---|---|---|
| Signals | **19** | public posts, each with a link back to the original |
| People | **19** | authors resolved from those posts |
| Outcomes | **1** | alerts a human marked actioned, ignored, or converted |
| Cohorts | **4** | live segments, membership recomputed on every read |
| Definitions | **7** | authoritative metric definitions |
| Agent runs | **16** | every one replayable, every step traced |
| Decisions | **3** | approval requests, each carrying the rule that answered it |

**100% of nodes carry provenance** (which agent wrote it, in which run, from what source). The target is 100, and anything below it is a bug.

Value to date: *Found 4 people publicly choosing a vendor. You acted on 1, and 1 became conversations.*

This number is small. It is supposed to be, at this stage. A graph that starts big was bought, and a bought graph is the one thing this is not.

<!-- GRAPH_STATS:END -->

## Why verifiability, not sophistication

Sophistication is the input, not the claim. The graph, the evals, the approval ledger, the provenance on every node: those exist so the system can be checked, and "checkable" is the thing a buyer is actually purchasing. A RevOps lead is not buying architecture. They are buying autonomy they will not be blamed for.

What every player in this category publishes as proof:

| | What they publish |
|---|---|
| Nevara | 20x fewer tokens, 10x faster shipping, >81% higher quality than generic agents |
| Warmly | A GTM Brain running an observe-orient-decide-act-learn loop |
| Nex | Agents that "get smarter every week" |
| Reo | +200% meetings, 40% attributed pipeline |
| Petavue | 2.5 hours down to 15 minutes per task |
| Clay | Glass-box reasoning traces |

Every one of those is an **outcome** claim. Not one is a **reliability** claim with a method behind it. No golden set, no precision and recall, no gate that blocks a release when quality drops.

That is the gap. It is also the one gap a small team can actually take, because rigor is a discipline rather than a headcount expenditure. You cannot out-build a funded team on architecture. You can absolutely out-prove them.

And it compounds in our favour: as this market fills with agents nobody can verify, provable reliability stops being a nice-to-have and becomes the filter buyers use.

**What it commits us to.** Publishing the number when it is bad. Hard-case sentiment sits at 0.571, sarcasm still reads wrong, and that line is in the table at the top of this README rather than in a footnote. A verifiability claim you only honour on good weeks is worth less than no claim at all.

## Why depth beats volume

The volume race is already lost, and it was the wrong race. Reo publishes 150M intent signals. Clay owns enrichment. ZoomInfo and Apollo have hundreds of millions of contacts. Competing there means buying the same data everyone else buys, which is why this README says plainly that **raw signals are not a moat**.

What nobody has is the other axis:

| Most GTM data | This graph |
|---|---|
| A signal | A signal, its source link, the agent that wrote it, and the run that produced it |
| A segment | A segment where every member states the reason it matched |
| An action | An action with the approval rule that permitted it |
| A contact record | A record with what it eventually produced: actioned, ignored, converted |
| Numbers in a dashboard | Numbers that cite the node they came from |

That is the outcome graph: the map of which signals, cohorts, definitions, and messages actually produced revenue. It compounds with every run, it is different for every customer, and it cannot be acquired. A rival can copy the architecture in a week. They cannot copy a year of your outcomes.

**The honest framing:** this is a direction, not a finished claim. The graph is small today. Every number in this README is generated from the running system rather than typed, so as it grows you can watch it, and if it stalls you will see that too.
