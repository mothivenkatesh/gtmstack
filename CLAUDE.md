...
**Design**
- Frappe Design System tokens (light surface, light sidebar). Body font is General Sans (self-hosted from `/fonts` as woff2, weights Regular/Medium/Semibold/Bold; Fontshare/ITF free license, so the public deploy is fine - no webfont-license caveat), code font is IBM Plex Mono (Google Fonts).
