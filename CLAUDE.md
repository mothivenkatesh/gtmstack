...
**Design**
- Frappe Design System tokens (light surface, light sidebar). Body font is Graphik LCG (proprietary, self-hosted from `/fonts` as woff2, true weights Regular/Medium/Semibold/Bold), code font is IBM Plex Mono (Google Fonts). Graphik is a licensed Commercial Type font: self-hosting on the public Vercel deploy redistributes it, which needs a webfont license.
