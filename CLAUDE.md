...
Backend module registry (mirror of the frontend manifests): `api/_registry.py` defines a `Module` base class (`id/name/desc`, `get(req)`/`post(req)` returning a uniform `Resp`, shared `_cron_ok` gate) and one class per feature (Transcript, Persona, Signals, Report, Monitor, Groups, Jobs, Clean, Plays, Auth, Watchdog) wrapping the `_*.py` engines; `REGISTRY = {id: instance}`. ALL endpoint semantics (param parsing, cron gating, CSV/JSON downloads, auth cookies + redirects) live in these classes, once. `app.py` is a single generic `/api/<id>` dispatcher over the registry; every Vercel `api/<id>.py` is a 3-line shim: `handler = make_handler("<id>")` via `api/_http.py`. Engine imports inside the classes are lazy so a function's cold start only pays for the engines it touches. Adding an endpoint = one Module class + one entry in `MODULES` + a 3-line `api/<id>.py` (plus a `vercel.json` functions entry if it needs includeFiles/maxDuration).
...
**Design**
- Frappe Design System tokens (light surface, light sidebar). Body font is Zoho Puvi (self-hosted from `/fonts` as woff2, weights Regular/Medium/Semibold/Bold; body copy defaults to Regular 400; Zoho distributes Puvi as a free download from zoho.com/puvi - verify the license terms before any external commercial launch), code font is IBM Plex Mono (Google Fonts).
