...
| Channels | GTMforce shell now; an MCP server + API later. |
...
| api/_llm.py | One provider-agnostic `chat()` shared by the persona + teardown engines: OpenAI-compatible endpoint (RunPod/OpenRouter/local) if `GTMFORCE_LLM_BASE_URL` set, else Anthropic, else `NoModel` (caller runs its heuristic). Strips `<think>` blocks |
...
- `creator_teardown` (Signals person posts -> a model names the hooks, formats, themes, cadence, and the one move to steal). The teardown engine reuses the persona LLM client path (`Anthropic()` on `ANTHROPIC_API_KEY`, model `GTMFORCE_MODEL`, JSON out) and degrades to a transparent heuristic when no key is set, so the step always completes.
...
Goal: add the second content-axis play, `creator_teardown` (a handle -> recent posts -> a model names the copyable patterns), and generalise the runner so it is schema-driven rather than hardcoded to the video play. This is the GTMforce form of Basis agent 13; the contact-axis Basis runs (agents 1-11) stay blocked on connectors (Phase 2).
...
| # | Change | File | Risk |
|---|---|---|---|
| 1 | New `_llm.py`: one `chat(system, user, max_tokens)` with a provider switch by env, most specific first. `GTMFORCE_LLM_BASE_URL` -> OpenAI-compatible (one `requests` POST to `{base}/chat/completions`, bearer `GTMFORCE_LLM_KEY`); else `ANTHROPIC_API_KEY` -> Anthropic; else raise `NoModel`. `configured()` for the gate. `_strip_think()` drops `<think>...</think>` so reasoning models' CoT does not pollute the JSON the caller extracts. Reasoning headroom: OpenAI path floors max_tokens at 3000 (override `GTMFORCE_MAX_TOKENS`). No new dep (reuses `requests`). | api/_llm.py | low |
...
| 3 | `.env` commented BYO-proxy + resilience tunables (`GTMFORCE_PROXIES`, `FETCH_*`). No secrets. | .env | none |
